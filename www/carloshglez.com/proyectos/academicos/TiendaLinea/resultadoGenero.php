...
<?php
	...
	else
	{
		...
		if($genero==1)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Drama' ORDER BY Nombre ASC",$link);
		else if($genero==2)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Comedia' ORDER BY Nombre ASC",$link);
		else if($genero==3)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Accion' ORDER BY Nombre ASC",$link);
		else if($genero==4)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Aventura' ORDER BY Nombre ASC",$link);
		else if($genero==5)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Terror' ORDER BY Nombre ASC",$link);
		else if($genero==6)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Romantico' ORDER BY Nombre ASC",$link);
		else if($genero==7)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Musical' ORDER BY Nombre ASC",$link);
		else if($genero==8)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Melodrama' ORDER BY Nombre ASC",$link);
		else if($genero==9)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Suspenso' ORDER BY Nombre ASC",$link);
		else if($genero==10)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Fantasia' ORDER BY Nombre ASC",$link);
		else if($genero==11)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Ciencia Ficcion' ORDER BY Nombre ASC",$link);
		else if($genero==12)
			$res=mysql_query("SELECT * FROM pelicula WHERE Genero='Animacion' ORDER BY Nombre ASC",$link);
...
<tr>
<td class="migas">Inicio &gt; Buscar Peliculas &gt; Resultado por Género</td>
</tr>
<tr>
<td><h2>Resultado de la Búsqueda por Género</h2></td>
</tr>
<tr>
<?php
...
<td height="220" valign="top">
	...
	for($i=$inicio; $i<=$fin; $i++)
	{
				...
				print("<td align=center><img src=".$reg[7]." border=0 width=30%   /></td>");
	...
	{
		$inicio = $fin + 1;
			$enlaceSig = "<a href=# class=mylink3 onClick=getText('resultadoGenero.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Genero=".$genero."&Subir=1&Bajar=0','contenido')>Siguiente<img src=imagenes/siguiente.png border=0 width=30 height=30/></a>";
	}if($in>0){
		if($fin==9)
			$inicio=0;
	 	else{
		...
		$enlaceAtr = "<a href=# class=mylink3 onClick=getText('resultadoGenero.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Genero=".$genero."&Subir=0&Bajar=1','contenido')><img src=imagenes/atras.png border=0 width=30 height=30/>Atras</a>";
	}
	?>
    </td>
        <?php
