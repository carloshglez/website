...
<?php
	...
	if($clasificacion==1)
		$res=mysql_query("SELECT * FROM pelicula WHERE Clasificacion='A' ORDER BY Nombre ASC",$link);
	else if($clasificacion==2)
		$res=mysql_query("SELECT * FROM pelicula WHERE Clasificacion='B' ORDER BY Nombre ASC",$link);
	else if($clasificacion==3)
		$res=mysql_query("SELECT * FROM pelicula WHERE Clasificacion='B-15' ORDER BY Nombre ASC",$link);
	else if($clasificacion==4)
		$res=mysql_query("SELECT * FROM pelicula WHERE Clasificacion='C' ORDER BY Nombre ASC",$link);
	else if($clasificacion==5)
		$res=mysql_query("SELECT * FROM pelicula WHERE Clasificacion='D' ORDER BY Nombre ASC",$link);
...
<tr>
<td class="migas">Inicio &gt; Buscar Peliculas &gt; Resultado por Clasificaci&oacute;n</td>
</tr>
<tr>
<td><h2>Resultado de la Búsqueda por Clasificaci&oacute;n</h2></td>
</tr>
<tr>
<?php
...
<td height="220" valign="top">
	...
	{
		$inicio = $fin + 1;
			$enlaceSig = "<a href=# class=mylink3 onClick=getText('resultadoClasificacion.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Clasificacion=".$clasificacion."&Subir=1&Bajar=0','contenido')>Siguiente<img src=imagenes/siguiente.png border=0 width=30 height=30/></a>";
	}if($in>0){
		if($fin==9)
			$inicio=0;
	 	else{
		...
		$enlaceAtr = "<a href=# class=mylink3 onClick=getText('resultadoClasificacion.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Clasificacion=".$clasificacion."&Subir=0&Bajar=1','contenido')><img src=imagenes/atras.png border=0 width=30 height=30/>Atras</a>";
	}
	?>
    </td>
        <?php
