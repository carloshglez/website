...
<?php
	...
	else
	{
		...
		$res=mysql_query("SELECT * FROM pelicula WHERE Nombre='".$nombre."'",$link);
...
<tr>
<td class="migas">Inicio &gt; Buscar Peliculas &gt; Resultado por Nombre</td>
</tr>
<tr>
<td><h2>Resultado de la Búsqueda por Nombre</h2></td>
</tr>
<tr>
    <?php
...
<td height="220" valign="top">
	...
	{
		$inicio = $fin + 1;
			$enlaceSig = "<a href=# class=mylink3 onClick=getText('resultadoNombre.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Nombre=".$nombre."&Subir=1&Bajar=0','contenido')>Siguiente<img src=imagenes/siguiente.png border=0 width=30 height=30/></a>";
	}if($in>0){
		if($fin==9)
			$inicio=0;
	 	else{
		...
		$enlaceAtr = "<a href=# class=mylink3 onClick=getText('resultadoNombre.php?Bandera=1&Inicio=".$inicio."&Fin=".($fin+1)."&Nombre=".$nombre."&Subir=0&Bajar=1','contenido')><img src=imagenes/atras.png border=0 width=30 height=30/>Atras</a>";
	}
	?>
    </td>
    <?php
